...
def test_takeltest_unit_jsonvars_is_not_none(jsonvars):
    assert jsonvars is not None


def test_takeltest_unit_jsonvars_no_localtemplates(
        templates,
        gather_molecule):
...
def test_takeltest_unit_jsonvars_no_gather_molecule(
        templates):
...
def test_takeltest_unit_jsonvars_resolve(
    ...
    my_template = {'unresolved': 'my_var',
                   'resolved': 'my_value',
                   'string': True}
    monkeypatch.setattr(takeltest.templates.Templates,
                        'resolve',
                        lambda x: None)
    monkeypatch.setattr(takeltest.templates.Templates,
                        'get_template',
...
def test_takeltest_unit_jsonvars_resolve_left_quote(
        jsonvars,
        monkeypatch):
    jsonvars.reset()
    my_template = {'unresolved': 'my_var',
...
def test_takeltest_unit_jsonvars_resolve_right_quote(
        jsonvars,
        monkeypatch):
    jsonvars.reset()
    my_template = {'unresolved': 'my_var',
...
def test_takeltest_unit_jsonvars_resolve_cache(
        jsonvars,
        monkeypatch):
    jsonvars.reset()
    my_template = {'unresolved': 'my_var',
...
def test_takeltest_unit_jsonvars_set(jsonvars):
    my_jsonvars = '{"my_var": "my_value"}'
