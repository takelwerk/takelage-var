import pytest
import takeltest
from takeltest.exceptions import TemplatesResolveError


def test_takeltest_unit_templates_is_not_none(templates):
    assert templates is not None


def test_takeltest_unit_templates_add(templates):
    templates.reset()
    templates.add('{{ my_var}}')
    assert templates._templates == [{'unresolved': '{{ my_var}}'}]


def test_takeltest_unit_templates_get_template(templates):
    templates.reset()
    templates.add('{{ my_var}}')
    assert templates.get_template(0) == {'unresolved': '{{ my_var}}'}


def test_takeltest_unit_templates_get_templates(templates):
    templates.reset()
    templates.add('{{ my_var}}')
    assert templates.get_templates() == [{'unresolved': '{{ my_var}}'}]


def test_takeltest_unit_templates_reset_templates(templates):
...
def test_takeltest_unit_templates_exception_templatesresolvefailed(
        templates,
        monkeypatch):
    playbook_results = []
    monkeypatch.setattr(takeltest.moleculebook.MoleculeBook,
...
def test_takeltest_unit_templates_add_debug_tasks(templates):
    task_0 = \
        {'action': {'module': 'debug',
...
def test_takeltest_unit_templates_exception_templatesresolveerror():
    msg = 'my_msg'
    with pytest.raises(TemplatesResolveError) as excinfo:
